...
the unpure version of it returned a string that we could use, the pure version
...
`IO[String]`, you can map over it to append a tranformation that turns the
...
entry point, meaning that nowhere in your code there's need for unpure
functions.
...
In the wild, in Scala, you'll rarelly see applications that use IO directly,
