...
images: /assets/posts/images/Effects.jpg
tags:
    - scala
    - haskell
    - learning
...
run-time in order to run the code), or that are dependent themselves from
observing external factors, break referential transparency, and so they have
side effects.. For example, in languages with the concept of class
...
represent, and allow us to perform effects, or as per our definition above, allows
us to perform side effects in a pure way. This might sound like a contradiction in
terms, but we'll explain in a moment.
...
while the implementation without `IO` was cooking the recipe directly.
